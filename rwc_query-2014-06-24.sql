...
LEFT JOIN phenotypes P2 ON P2.id = PP2.phenotype_id AND P2.value_id = 69 AND P2.entity_id = 366 AND ((P2.date <= (P1.date + INTERVAL 7 DAY)) AND (P2.date >= (P1.date + INTERVAL 1 DAY))) 
LEFT JOIN phenotype_plants PP3 ON PP3.plant_id = Base.plant_id 
LEFT JOIN phenotypes P3 ON P3.id = PP3.phenotype_id AND P3.value_id = 156 AND P3.entity_id = 366 AND ((P3.date <= (P1.date + INTERVAL 5 DAY)) AND (P3.date >= (P1.date + INTERVAL 1 DAY))) 
WHERE P1.number IS NOT NULL AND P1.invalid IS NULL AND P2.number IS NOT NULL AND P2.invalid IS NULL AND P3.number IS NOT NULL AND P3.invalid IS NULL
